...
Staff ↔ group: `Instellingen.User.UsrGroepId` → `StaffUser.UserGroupId` → `[Instellingen].[UsrGroep]` (`UserGroup`). Admin UI: `/admin/staff-users` (password + group + Admin/Manager) and `/admin/user-groups`. No separate My profile � staff data only on Staff user.
