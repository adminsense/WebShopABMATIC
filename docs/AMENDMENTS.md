...
> **2026-07-29 — Admin forms + attribute values as modal:** Removed global `max-width: 640px` on `.entity-form-fields` (both `admin.css` copies); admin CRUD/workflow forms now match grid width with redistributed Bootstrap columns. Product attribute values moved from a separate screen to the **content-modal pattern**: new reusable `ProductAttributeValuesModal` (Add + inline edit + stacked delete confirm) opened from the Attributes action on `/admin/products` and on the `/admin/product-attributes` selection grid; **Close** returns to the calling grid with no navigation; deep link `/admin/product-attributes/{ProdId}` opens the modal over the grid. Documented as pattern §6.1 in [PATTERNS_UI_QUICK_START.md](./PATTERNS_UI_QUICK_START.md) + agent rule `.cursor/rules/ui-patterns-first.mdc`. Login/modals stay narrow.
