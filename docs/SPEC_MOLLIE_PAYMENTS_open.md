...
> **Purpose of this doc:** (1) document the **approved mock** path used now; (2) ops checklist for **B.9** only after keys arrive.  
...
- Checkout PrePay uses **`/checkout/mollie-mock`** (demo card / iDEAL bank UI).
- Stock / reservation / payment-return behaviour is exercised against **mock** payment ids (`tr_mock_*`).
...
### Checkout behaviour (mock)

- `CheckoutUseCase` PrePay path + `MollieMockPaymentAdapter`
- Webhook endpoint exists: `POST /api/webhooks/mollie/payments` (used for real Mollie later; mock path settles via mock page + payment-return)
- **D.7** — reserves stock on PrePay order; sale after “paid”

### UI `/cart`

- Cart requires **customer** sign-in (`IsCustomerCart`)
- Delivery address + payment method (default **iDEAL / card (Mollie)**)
- **Place order & pay** blocked when quote has stock errors (see [SPEC_WEB_STORE.md](./SPEC_WEB_STORE.md) §5.2)

### Mock payment UX

- Redirect to **`/checkout/mollie-mock`** → customer clicks **Pay** → `/orders/{id}/payment-return`
- Mock ids treated as **paid** so stock and audit run locally

**Demo card (pre-filled):** `4111 1111 1111 1111` · Demo Customer · `12/28` · CVC `123`

**Static reference:** [docs/mocks/mock-payments.html](./mocks/mock-payments.html)
...
- `PaymentMethods` row **iDEAL / card (Mollie)** with `IsPrePay = 1`
...
2. Cart → **Place order** with **iDEAL / card (Mollie)**.
3. Redirect to **real** Mollie hosted checkout.
4. Pay with Mollie test method.
5. Verify webhook / payment-return, paid status, stock, admin order column, audit.
6. Repeat thoughtfulness for cancel/expire → reservation release (already coded).

### 4. Payment return fallback

If webhook is delayed, payment-return / order detail polls Mollie and applies the same paid + stock logic.
...
- [SPEC_WEB_STORE.md](./SPEC_WEB_STORE.md) — checkout / cart
