...
> **Runtime amendment (2026-07-14) — Azure Blazor + store auth:** Removed in-memory `StoreBrowserSessionStore` (cookie alone authorizes customers). Interactive Server **prerender on**. Azure App Service must set **Web sockets = On** (Immo production already has this; see [SPEC_INFRASTRUCTURE.md](../SPEC_INFRASTRUCTURE.md) troubleshooting). Idle logout remains client-side 15 min → `/account/logout`.

> **Runtime amendment (2026-07-14) — catalog price/stock UI:** Guests see **list price** (or **Out of stock** / **Price on request**). Removed “Meld u aan om uw prijs te zien” from cards. Login only when buying (add to cart). See [SPEC_WEB_STORE.md](../SPEC_WEB_STORE.md) §5.1 / §9.1.
...
| Imagens | `AzureFile` + blob `files` | ✅ [DATA_AZUREBLOB.md](DATA_AZUREBLOB.md) |

### 3.5 Formulários por produto e categoria (evolução — análise)

> [!NOTE]
> **Estado:** ✅ **implementado (2026-07-10)**. Opções de produto, texto intro/outro de categoria, opções no carrinho **e persistência na encomenda** estão ligados à BD `abmatic_test`. As opções são gravadas em `OrderLines` como **linhas-filhas** (`IsOption=1`, `ProductOptionId`, `ProductOptionHoofdDetaillId` = Id da linha do produto) com `PopUpDataXml` a registar a selecção. ⚠️ **A confirmar:** o formato segue o padrão inferido pelos campos legacy; validar contra uma encomenda real do ERP (linha-filha vs XML) e ajustar se necessário.

Na app de referência e no ERP AB-MATIC, produtos e categorias não são só listagem — trazem **campos configuráveis**, textos e opções que o comprador preenche antes de adicionar ao carrinho. O admin **já gere** estes dados (`ProductOption`, `ProductStructure`, textos de lista de preços); a loja **passa a expor** essas opções e textos no layout novo, lendo directamente da BD.

| Âmbito | Dados / origem (admin + ERP) | Loja hoje | Fase C (ports/DTOs) | Fase D (UI) |
|--------|------------------------------|-----------|--------------------------|------------------|
| **Produto — opções** | `ProductOption` (+ `ProductOptionValue`, obrigatório/opcional) — ver [SPEC_WEB_STORE.md](SPEC_WEB_STORE.md) §3 | ✅ Detalhe mostra opções (dropdown/texto) e bloqueia Add to cart se obrigatória em falta | ✅ `GetProductOptionsAsync(productId)` + `StoreProductOptionDto`/`StoreProductOptionValueDto` (EN→NL fallback) | ✅ `StoreProductOptionsForm.razor` no detalhe; validação de obrigatórias |
...
| **Categoria — contexto** | `ProductStructuur`: `NameEn`, `IntroPriceListTextId`, `OutroPriceListTextId`, ícone | ✅ Cabeçalho da main mostra H1 + texto intro; outro no fim da secção | ✅ `GetCategoryDetailAsync(categoryId)` + `StoreCategoryDetailDto` (intro/outro via `PrijslijstTeksten`, EN→NL) | ✅ `StoreCategoryHeader` (intro) + outro em `Catalog.razor` |
...
| **Carrinho / encomenda** | Linhas com opções seleccionadas | ✅ `CartLine.Options`; linhas iguais só juntam se opções iguais (`Signature`) | ✅ Opções fluem carrinho→`CheckoutLineRequest`→`StoreOrderLineCreate`; gravadas em `OrderLines` (linha-filha) | ✅ Resumo por linha no `Cart.razor`; persistência em OrderLines ⚠️ formato a validar |
...
**Origem dos dados (BD `abmatic_test`, read-only na vertical da loja):**

| Entidade BD | Tabela legacy | Uso na loja |
|-------------|---------------|-------------|
| `ProductOption` / `ProductOptionValue` | `[Products].[ProductOptions]` / `[Products].[ProductOptionValue]` | Opções + valores por `ProductId` (`GetProductOptionsAsync`) |
| `ProductStructure` | `[Products].[ProductStructuur]` | Nome + `IntroPriceListTextId` / `OutroPriceListTextId` |
| `PriceListText` | `[Products].[PrijslijstTeksten]` | Texto intro/outro (EN→NL fallback) |
| `Product` + preços + stock | — | Já usado em `StoreCatalogService` |

**Critérios de aceitação (estado):**

- ✅ Detalhe do produto mostra **todas** as opções e bloqueia “Add to cart” enquanto uma obrigatória estiver em falta.
- ✅ Filtro de categoria mostra **cabeçalho** com nome + texto intro quando existir na BD (outro no fim).
- ✅ Opções seleccionadas seguem para o **carrinho** (resumo por linha); persistência em OrderLines fica para fase backend.
...
- ✅ DTOs store: `StoreProductOptionDto` + `StoreProductOptionValueDto` (`IsRequired`, `SortOrder`, `ValueType`) e `StoreCategoryDetailDto`
- ✅ `GetProductOptionsAsync(productId)` — read-only de `ProductOptions` + `ProductOptionValue` (EN→NL fallback, ordenado)
- ✅ `GetCategoryDetailAsync(categoryId)` — nome + textos intro/outro via `PrijslijstTeksten` (`IntroPriceListTextId` / `OutroPriceListTextId`)
- ✅ `StoreProductDto.HasOptions` — flag para quick-add encaminhar ao detalhe quando há opções
- ✅ Carrinho: `CartLine.Options` + `Signature` (linhas iguais só juntam com opções iguais); selecção captada no cliente
- ✅ Pipeline de encomenda: `CheckoutLineRequest.Options` → `StoreOrderLineCreate.Options` → `StoreOrderRepository`
- ✅ Persistência em `OrderLines`: linha-filha por opção (`IsOption=1`, `ProductOptionId`, `ProductOptionHoofdDetaillId`, `PopUpDataXml`) — ⚠️ formato inferido dos campos legacy, **a validar** com encomenda real do ERP
...
**Fase D.2 — Formulários na UI loja (✅ 2026-07-10)** — ver §3.5

- ✅ `StoreProductOptionsForm.razor` — opções por produto (dropdown para lista de valores, campo texto caso contrário)
- ✅ `ProductDetail.razor` — integra formulário + validação (Add to cart bloqueado até obrigatórias preenchidas) + envia opções ao carrinho
- ✅ Cabeçalho de categoria na main: H1 + **texto intro**; **outro** no fim da secção (`StoreCategoryHeader` + `Catalog.razor`)
- ✅ `ProductCartButton` — quick-add encaminha para o detalhe quando o produto tem opções (`HasOptions`)
- ✅ `Cart.razor` — mostra opções seleccionadas por linha (`OptionsSummary`)
- ✅ Estilos em `store.css` (`.product-options`, `.product-option-*`, `.cart-line-options`, `.store-category-intro/outro`)
...
[Bestanden].[AzureFile]        → imagens (DATA_AZUREBLOB.md)
```
...
| 9 | Formulários produto/categoria na loja? | ✅ Entregue — §3.5; Fase C.2 + D.2 (2026-07-10) |
...
- 🖼️ [DATA_AZUREBLOB.md](DATA_AZUREBLOB.md) — imagens  
...
Only when you have credentials and a public URL. See [SPEC_MOLLIE_PAYMENTS_open.md](SPEC_MOLLIE_PAYMENTS_open.md).
