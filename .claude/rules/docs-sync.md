...
- Prefer short factual notes. **`AMENDMENTS.md`:** one table per calendar day in plain GitHub Markdown (no HTML), with the date as the single left-aligned column header (`|:---|`) and one numbered change per row. Update today’s table instead of creating another table for the same date. Archive bulky history under `docs/archive/`.
