...
- ⬜ **6.3** Bridge table or mapping: Identity user → legacy `StaffUsers.Id` where int FK required
...
- ⬜ **7.1** Design `AuditLog` entity or use existing `OrderLog` pattern with Identity user id
- ⬜ **7.2** Middleware or domain hook on admin mutations
- ⬜ **7.3** Admin read-only audit journal (optional)
