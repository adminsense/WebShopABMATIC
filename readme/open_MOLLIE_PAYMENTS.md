...
- PrePay checkout creates a `tr_mock_*` payment id and redirects to **`/checkout/mollie-mock`** — simulated Mollie hosted page with **pre-filled demo card** (or iDEAL bank). Customer clicks **Pay** → `/orders/{id}/payment-return`.
...
**Static reference:** [docs/mock-payments.html](docs/mock-payments.html) — same UX as the in-app page.

To switch to real Mollie: set `Mollie:ApiKey`, set `Mollie:UseMock` to `false`, expose a public webhook URL — customer then sees **real** Mollie hosted checkout instead of `/checkout/mollie-mock`.

This completes **dev** checkout for client demos. **B.9 prod** remains ⬜ until you configure a real test key and run the checklist below.
