...
public sealed class VatTypeRepository : IVatTypeRepository
{
    private readonly WebShopABMATICDbContext _db;

    public VatTypeRepository(WebShopABMATICDbContext db) => _db = db;
    ...
    public async Task<PagedResult<VatTypeDto>> GetVatTypesAsync(VatTypeListFilter filter, CancellationToken cancellationToken = default)
    {
        ...
        if (!string.IsNullOrWhiteSpace(filter.Search))
        {
            ...
            {
                query = query.Where(e => e.Id == id || e.Name.Contains(term));
            }
            else
            {
